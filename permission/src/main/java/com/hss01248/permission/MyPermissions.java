...
public class MyPermissions {
    ...
    public static boolean isStateInManifest(String permission) {
        try {
            ...
            PackageInfo packageInfo = Utils.getApp().getPackageManager().getPackageInfo(AppUtils.getAppPackageName(),
    ...
    @Nullable
    String afterDialogTitle;// ""--> 不要title, null -> 使用默认title
    @Nullable
    String afterPermissionMsg;
    @Nullable
    String guideToSettingMsg;
    ...
    private static List<String> getDeniedForeverList(String[] permission) {
        ...
        for (String per : permission) {
            if (!PermissionUtils.isGranted(per)
                    && !ActivityCompat.shouldShowRequestPermissionRationale(ActivityUtils.getTopActivity(), per)) {
                ...
                .callback(new PermissionUtils.FullCallback() {
                    ...
                    public void onDenied(@NonNull List<String> deniedForever, @NonNull List<String> denied) {
                        ...
                        if (hasShowSysDialog) {
                            ...
                            // 修复bug: 对于从未请求过的权限,shouldShowRequestPermissionRationale返回false,
                            // 会被错误地放入deniedForeverList。所以不能用请求前的deniedForeverList来做交集判断。
                            // 正确逻辑: 显示过系统弹窗后,如果denied(暂时拒绝)不为空,说明用户本次只是点击拒绝,没有勾选"不再询问",
                            // 此时应该走checkIfRetryAfterFirstTimeRequest流程,而不是跳转到设置页。
                            // 只有当denied为空且deniedForever不为空时,才需要判断是否本身就是永久拒绝。
                            if (!denied.isEmpty()) {
                                // 有暂时拒绝的权限,说明用户本次点击了拒绝但没有勾选"不再询问"
                                LogUtils.i("有暂时拒绝的权限,用户本次点击了拒绝,走挽留流程");
                                checkIfRetryAfterFirstTimeRequest(deniedForever, denied);
                            } else if (!deniedForever.isEmpty()) {
                                // denied为空,deniedForever不为空,需要判断是本次永久拒绝还是本身就永久拒绝
                                // 使用请求前的deniedTemporary来辅助判断:
                                // 如果请求前deniedTemporary包含某权限,说明该权限之前是"暂时拒绝"状态,
                                // 现在变成deniedForever,说明用户本次勾选了"不再询问"
                                Collection intersection = CollectionUtils.intersection(deniedTemporary, deniedForever);
                                if (!intersection.isEmpty()) {
                                    // 有权限从暂时拒绝变成永久拒绝,说明是用户本次操作的结果
                                    LogUtils.i("有权限从暂时拒绝变成永久拒绝,是用户本次勾选了不再询问,走挽留流程");
                                    checkIfRetryAfterFirstTimeRequest(deniedForever, denied);
                                } else {
                                    // 请求前的deniedTemporary和deniedForever没有交集,说明这些权限本身就是永久拒绝的
                                    LogUtils.w("权限本身就是永久拒绝状态,跳转到设置页面");
                                    goSettingFirstTimeWrapper(false, deniedForever);
                                }
                            } else {
                                // denied和deniedForever都为空,理论上不会走到这里,因为onDenied回调表示有权限被拒绝
                                LogUtils.w("异常情况: denied和deniedForever都为空");
                                callback.onDenied(deniedForever, denied);
                            }
                            ...
                            LogUtils.w(
                                    "第一次_全部都是永久拒绝_直接gosetting_小于1s,说明没有显示系统权限弹窗,直接调用了denied,且全部是deniedForever,或者没有在manifest里声明权限");
                            // LogUtils.w("但不适用于精确和模糊定位同时请求,且拒绝了精确,选择了模糊定位权限的情况,此时如果选择了not show after
                            // dialog,也会走到这里,需要做兼容处理");
     ...
     */
    private static boolean dealDeniedList(List<String> deniedForever, List<String> denied,
            long sysPermissionDialogShowTime) {
    ...
    private void checkIfRetryAfterFirstTimeRequest(@NonNull List<String> deniedForeverList,
            @NonNull List<String> deniedTemporary) {
        // 准备重试
        /*
         * List<String> deniedForeverList = getDeniedForeverList(permissions);
         * List<String> deniedTemporary = getDeniedTemporary(permissions);
